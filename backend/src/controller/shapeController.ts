...
const findClosestIndex = (coordinates: { lat: number; lng: number }[], lat: number, lng: number): number => {
...
export const getShapeById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const coordinates = shape.coordinates as { lat: number; lng: number }[];
    ...
    if (originStopId && destStopId) {
      const [originStop, destStop] = await Promise.all([
        db.getStopById(originStopId as string),
        db.getStopById(destStopId as string),
      ]);
      ...
      if (originStop && destStop) {
        ...
        console.log(
          `Shape ${lineId}: ${fullLength} → ${trimmedCoords.length} pts ` +
...
export const getShapeByRouteNumber = async (req: Request, res: Response): Promise<void> => {
  try {
    const routeNumber = req.params['routeNumber'] as string;
