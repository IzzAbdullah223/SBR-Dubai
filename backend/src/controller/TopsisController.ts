...
const MODE_WEIGHTS: Record<string, Weights> = {
  ...
  try {
    ...
    if (routeData.directRoutes.length === 0 && routeData.transferRoutes.length === 0) {
      res.status(404).json({
        ...
        errorCode: 'NO_ROUTES',
        message:
          'Bus stops were found near both locations, but no routes connect them. ' +
    ...
    if (allBuses.length === 0) {
      res.status(404).json({
        ...
        message:
          'Routes were found but no buses are currently running. ' +
