...
const updateRoutesWithShapes = async () => {
  ...
  try {
    ...
    const routeShapes = new Map<string, Record<string, string>>();
    ...
    for (const route of allRoutes) {
      ...
      console.log(
        ` Route ${route.routeNumber.padEnd(6)} → forward: ${(forwardLineId ?? 'none').padEnd(12)} return: ${returnLineId ?? 'none'}`
      );
      updated++;
    }
