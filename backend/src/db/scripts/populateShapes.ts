...
const importShapes = async () => {
  ...
  try {
    ...
    const shapesMap = new Map<string, { points: { lat: number; lng: number; sequence: number; distTraveled: number }[]; maxDist: number }>();
    ...
    for (const row of records) {
      ...
      if (!shapesMap.has(shapeId)) shapesMap.set(shapeId, { points: [], maxDist: 0 });

      const entry = shapesMap.get(shapeId)!;
      entry.points.push({ lat, lng, sequence, distTraveled });
      if (distTraveled > entry.maxDist) entry.maxDist = distTraveled;
    }
