...
export const findDirectRoutes = async (
  ...
  const connectingRoutes: DirectRoute[] = []
  ...
  allRoutes.forEach(route => {
    ...
    if (!matchingOriginStops.length || !matchingDestStops.length) return
...
export const findRoutesWithTransfer = async (
  originStops: NearbyStop[],
  destStops:   NearbyStop[]
  ...
  const allCommonStopIds = new Set<string>()
  ...
  const stopPositionMap = new Map(
    commonStopDetails.map(s => [s.stopId, { lat: s.lat, lng: s.lng }])
  )
  ...
  routesFromOrigin.forEach(route1 => {
    routesToDest.forEach(route2 => {
      ...
      if (!commonStopIds.length) return
