...
function determineStopType(name) {
    ...
    if (n.includes('station') || n.includes('metro'))
        return 'station';
    if (n.includes('terminal') || n.includes('bus stn'))
        return 'terminal';
    return 'stop';
}
function determineRouteType(routeNumber) {
    if (routeNumber.startsWith('X'))
        return 'express';
    if (routeNumber.startsWith('E'))
        return 'intercity';
    if (routeNumber.startsWith('C') || routeNumber.startsWith('F'))
        return 'local';
    return 'local';
}
...
async function processStopTimes(routes, routeTripsMap) {
    ...
    routes.forEach(route => {
        ...
        if (first)
            neededTripIds.add(first.tripId);
    });
    ...
    routes.forEach(route => {
        ...
        if (tripStops.length === 0)
            return;
        routeStopsMap.set(route.routeId, tripStops.map((s, idx) => ({ stopId: s.stopId, stopSequence: idx + 1 })));
    });
...
//  imprt routes
async function importBusRoutes(routes, routeStopsMap, stopCoordsMap) {
...
async function updateStopsWithRoutes(routes, routeStopsMap) {
    console.log('Updating stops with route assignments...');
    const stopRoutesMap = new Map();
    for (const route of routes) {
        const stops = routeStopsMap.get(route.routeId) ?? [];
        for (const stop of stops) {
            if (!stopRoutesMap.has(stop.stopId))
                stopRoutesMap.set(stop.stopId, []);
...
main();
//# sourceMappingURL=seedDubaiGTFS.js.map